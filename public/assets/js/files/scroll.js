...
//===============================================================
function parallax() {
	let parallaxObj = document.querySelector('.main__bacground');

	if (!parallaxObj) return;

	let parallaxBlock = document.querySelector('.main');

	const parallaxBlockRect = parallaxBlock.getBoundingClientRect();

	const parallaxBlockHeight = parallaxBlockRect.height;
	const parallaxBlockTop = parallaxBlockRect.top;
	const parallaxBlockBottom = parallaxBlockRect.bottom;
	...
	const parallaxLength = parallaxBlockHeight - windowHeight;
	const parallaxStep = scaleRange / parallaxLength;

	const scaleValue = 1 - (parallaxStep * windowTop);

	const translateValue = parallaxBlockBottom - windowHeight;

	if (translateValue > 0) {
		parallaxObj.style.transform = `scale(${scaleValue > scaleMin ? scaleValue : scaleMin})`;
	} else {
		parallaxObj.style.transform = `translateY(${translateValue}px) scale(${scaleValue > scaleMin ? scaleValue : scaleMin})`;
	}

	// console.clear();
	// console.log('parallaxBlockTop:' + parallaxBlockTop);
	// console.log('parallaxBlockBottom:' + parallaxBlockBottom);
	// console.log('windowTop:' + windowTop);
	// console.log('windowBottom:' + windowBottom);
	// console.log('parallaxLength:' + parallaxLength);
	// console.log('parallaxStep:' + parallaxStep);
...
//===============================================================
function initCounterTargets() {
	const counterElements = document.querySelectorAll('[data-counter-target]');

	if (!counterElements.length) return;

	counterElements.forEach(counterElement => {
		new ClassWatcher(counterElement, 'aos-animate', () => {
			const counterNumber = +counterElement.dataset.counterTarget;
			const counterStep = +counterElement.dataset.counterStep;
			const duration = 5000;
			const delta = 50;

			const counterInterval = duration / (counterStep * delta);

			let counter = 0;

			let intervalId = setInterval(() => {
				counterElement.innerHTML = counter.toLocaleString(undefined, { minimumFractionDigits: 0 });
				counter += counterStep;
				
				if (counter > counterNumber) {
					counterElement.innerHTML = counterNumber.toLocaleString(undefined, { minimumFractionDigits: 0 });;
...
document.addEventListener('DOMContentLoaded', () => initCounterTargets());
